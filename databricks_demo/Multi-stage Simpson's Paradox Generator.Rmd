...
[Simpson’s paradox](https://en.wikipedia.org/wiki/Simpson's_paradox) is the statistical phenomenon whereby the association between a pair of variables (X, Y) reverses sign when we condition on a third variable (Z). It is called a paradox because people often find the results counterintuitive.
...
mssp <- results$par %>% simulate_with_pars(node_parents, num_rows=1000)

mssp_test <- results$par %>% simulate_with_pars(node_parents, num_rows=1000) # independent test set for later use
...
sensitivity_df %>% 
  ...
  ggplot(aes(x=delta_X, y=delta_Y, col=predictors)) + geom_point(size=5, alpha=0.8)
```
...
def evaluate_delta(delta_X, my_inputs):
  outcome = mssp['Y'].copy()
  predictors = mssp[ my_inputs ].copy()
  ...
  return {'inputs': '+'.join(my_inputs), 'delta_X': delta_X, 'delta_Y': delta_Y}


result_rows = []
for num_inputs in range(1, len(inputs)+1):
  these_inputs = inputs[0:num_inputs]
  for delta_X in np.linspace(-50, 50, 21):
    result_rows.append( evaluate_delta(delta_X, these_inputs) )
```
...
results.plot.scatter(x='delta_X', y='delta_Y', c=[input_colors[x] for x in results['inputs']])

i = 0
for c in input_colors.keys():
  plt.text(-45, -(10+i), c, color=input_colors[c])
  i += 2
```
